...
export class CustomProvider implements AIProvider<'custom'> {
	...
	private get url(): string {
		return configuration.get('ai.experimental.custom.url') || 'http://localhost:3000/v1/chat/completions';
	}
	...
	private async getOrChooseModel(): Promise<CustomModels | undefined> {
		...
		await configuration.updateEffective(`ai.experimental.${pick.provider}.model`, 'custom');
		return 'custom';
	}

	async generateCommitMessage(diff: string, options?: { context?: string }): Promise<string | undefined> {
		// An API Key may not be required but might still be necessary depending on various internal setups at Enterprise
...
Do not make any assumptions or invent details that are not supported by the code diff or the user-provided context.`,
	...
	private fetch(apiKey: string | undefined | null, request: CustomChatCompletionRequest) {
		const url = this.url;
...
export type CustomModels = 'custom';

// The Request and Response are copied from OpenAi
