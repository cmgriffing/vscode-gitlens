...
const provider = { id: 'otherai', name: 'OtherAI' } as const;
...
type OtherAIModel = AIModel<typeof provider.id>;
const models: OtherAIModel[] = [
	{
		id: 'gpt-4-turbo',
...
export class OtherAIProvider implements AIProvider<typeof provider.id> {
	...
	private async getOrChooseModel(): Promise<OtherAIModel | undefined> {
...
Do not make any assumptions or invent details that are not supported by the code diff or the user-provided context.`,
	...
	private async fetch(
		apiKey: string,
		request: OtherAIChatCompletionRequest,
